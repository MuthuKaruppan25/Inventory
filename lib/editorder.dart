        ...
        actions: [
          IconButton(onPressed: ()async{
            ...
            if(q==1 && w==1 && e==1 && u==0 && o==0 ) {
              ...
              for(int i=0;i<order.length;i++)
              {
                DocumentSnapshot data = await FirebaseFirestore.instance.collection("Universal").doc(order[i][0].toString()).get();
                await FirebaseFirestore.instance.collection("Universal").doc(order[i][0].toString()).update({
                  "quantity":data["quantity"]+int.parse(order[i][1].toString())
                });
              }
              for(int i=0;i<orders.length;i++)
